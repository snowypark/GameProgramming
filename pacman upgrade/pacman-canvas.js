	...
	function Game() {
		...
		this.nextLevel = function () {
			...
			if (this.level === FINAL_LEVEL) {
				console.log('다음 레벨, ' + FINAL_LEVEL + ', end game');
				game.endGame(true);
				game.showHighscoreForm();
			} else {
				this.level++;
				console.log("Level " + game.level);
				game.pauseAndShowMessage("Level " + game.level, this.getLevelTitle() + "<br/>(클릭 시 계속!)");
		...
		this.forcePause = function () {
			this.timer.stop();
			this.pauseAndShowMessage("일시 정지", "클릭 시 시작");
		}
		...
		this.buildWalls = function () {
			if (this.ghostMode === 0) game.wallColor = "rgb(123, 220, 190";
			else game.wallColor = "gray";
